...
object BlockingIOs extends IOApp.Simple {
  ...
  val someSleeps = for {
    _ <- IO.sleep(1.second).debugging // SEMANTIC BLOCKING
    _ <- IO.sleep(1.second).debugging
  } yield ()
  ...
  val iosOnManyThreads = for {
    _ <- IO("first").debugging
    _ <-
      IO.cede // a signal to yield control over the thread - equivalent to IO.shift
    _ <- IO(
      "second"
    ).debugging // the rest of this effect may run on another thread (not necessarily)
    _ <- IO.cede
    _ <- IO("third").debugging
  } yield ()
  ...
  def testThousandEffectsSwitch() = {
    ...
    (1 to 1000)
      .map(IO.pure)
      .reduce(_.debugging >> IO.cede >> _.debugging)
      .evalOn(ec)
  }
