...
object Effects {
  ...
  // referential transparency = can replace an expression with its
  // value as many times as we want without changing behavior
  ...
  /*
    ...
    Properties:
      ...
      - when side effects are needed, effect construction is separate from effected execution
  */
  ...
  */
   ...
   * 2. An IO which measures the duration of a computation (hint: use ex1)
  ...
  /*
    clock.flatMap(
      startTime => computation.flatMap(
                      _ => clock.map(finishTime => finishTime - startTime)
      )
    )

    clock.flatMap(
      startTime => computation.flatMap(
                        _ => MyIO(() => System.currentTimeMillis - startTime)
      )
    )
