...
object Semaphores extends IOApp.Simple {
  ...
  def login(id: Int, sem: Semaphore[IO]): IO[Int] = for {
    _ <- IO(s"[session $id] waiting to log in...").debugging
    _ <- sem.acquire
    // critical section
    _ <- IO(s"[session $id] logged in, working...").debugging
    res <- doWorkWhileLoggedIn()
    _ <- IO(s"[session $id] done: $res, logging out...").debugging
  ...
  /** Exercise:
    *   1. find out if there's something wrong with this code 2. why 3. fix it
    */
  ...
  override def run: IO[Unit] = usersFixed.debugging.void
